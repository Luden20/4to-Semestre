...
def abrirArchivo():
    ...
    if archivo:
        ...
        revisiones = [sheet for sheet in df.sheet_names if "Revision" in sheet]
        combo_revision['values'] = revisiones
        if revisiones:
            combo_revision.set(revisiones[0])  # Seleccionar la primera revisión por defecto
...
def reemplazar(texto, n, df_revision):
    ...
    texto = texto.replace("<<Sílabo>>", safe_str(df_revision['Sílabo'][n]))
    texto = texto.replace("<<Información>>", safe_str(df_revision['Información'][n]))
    texto = texto.replace("<<Herramientas>>", safe_str(df_revision['Herramientas'][n]))
    texto = texto.replace("<<Organización>>", safe_str(df_revision['Organización'][n]))
    texto = texto.replace("<<Recursos>>", safe_str(df_revision['Recursos'][n]))
    texto = texto.replace("<<Rubricas>>", safe_str(df_revision['Rubricas'][n]))
    texto = texto.replace("<<Evaluación>>", safe_str(df_revision['Evaluación'][n]))
    texto = texto.replace("<<Calificador>>", safe_str(df_revision['Calificador'][n]))
    try:
...
def generar_archivos():
    label_estado.config(text="Por favor, espere...", foreground="blue")
    ventana_principal.update_idletasks()

    revision_seleccionada = combo_revision.get()
    archivo = ubicacion_archivo.get()
    df_revision = pd.read_excel(archivo, sheet_name=revision_seleccionada)
    df_revision.columns = ['Apellidos', 'Nombres', 'Facultad', 'Carrera', 'NRC', 'Fecha', 'Hora', 'Sílabo',
                           'Información', 'Herramientas', 'Organización', 'Recursos', 'Rubricas', 'Evaluación',
                           'Calificador']
    df_revision['Nombre Completo'] = df_revision['Apellidos'] + " " + df_revision['Nombres']
    lista_docentes = df_revision['Nombre Completo'].unique()

    seguimiento_path = os.path.join(ubicacion_carpeta_principal.get(), revision_seleccionada)
    ...
    for n in range(df_revision.shape[0]):
        ...
        archivo_path = os.path.join(docente_path, f"{docente}_{int(df_revision['NRC'][n])}-{revision_seleccionada}.tex")
        ...
        dir_archivo_sin_ext = os.path.join(docente_path, f"{docente}_{int(df_revision['NRC'][n])}-{revision_seleccionada}")
    ...
    label_estado.config(text="Archivos generados exitosamente", foreground="green")
...
etiqueta3 = ttk.Label(text="Seleccione la revisión")
etiqueta3.grid(column=0, row=5, pady=(0, 2))

combo_revision = ttk.Combobox(ventana_principal, state='readonly')
combo_revision.grid(column=0, row=6, pady=(2, 20))
...
progreso.grid(column=0, row=8, pady=(10, 20))

# Etiqueta de estado
label_estado = ttk.Label(text="")
label_estado.grid(column=0, row=9, pady=(10, 20))
