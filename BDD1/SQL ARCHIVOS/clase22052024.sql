...
LEFT JOIN factura f
ON c.cli_cedula_ruc=f.cli_cedula_ruc 
WHERE f.fac_numero IS NULL
ORDER BY fac_numero;
--RIGHT JOIN
select c.cli_nombre,f.fac_numero,f.fac_monto from factura f
RIGHT JOIN cliente c
ON c.cli_cedula_ruc=f.cli_cedula_ruc 
WHERE f.fac_numero is NULL
ORDER BY fac_numero;

DESC FORM_PAG_FACTURA;
SELECT C.CLI_NOMBRE,F.FAC_NUMERO,FP.FRP_CODIGO
FROM FACTURA F
INNER JOIN FORM_PAG_FACTURA FP
ON F.FAC_NUMERO=FP.FAC_NUMERO
INNER JOIN CLIENTE C
ON C.CLI_CEDULA_RUC=F.CLI_CEDULA_RUC;

DESC SUCURSAL;

SELECT S.SUC_NOMBRE,C.CLI_NOMBRE,F.FAC_NUMERO,F.FAC_FECHA
FROM SUCURSAL S
INNER JOIN FACTURA F
...
/* Table: CLIENTE                                               */
/*==============================================================*/
create table HIS_CLIENTE (
   CLI_CEDULA_RUC       VARCHAR2(13)          not null,
   CIU_CODIGO           VARCHAR2(3)           not null,
   CLI_NOMBRE           VARCHAR2(50),
   CLI_APELLIDO         VARCHAR2(50),
   CLI_CORREO           VARCHAR2(60),
   CLI_TELEFONO         VARCHAR2(13),
   CLI_DIRECCION        VARCHAR2(60),
   CLI_SECTOR           VARCHAR2(60),
   constraint PK_HIS_CLIENTE primary key (CLI_CEDULA_RUC)
)
   tablespace DATOS_2024_01
/
ALTER TABLE HIS_CLIENTE MODIFY (CIU_CODIGO VARCHAR2(5));
INSERT INTO HIS_CLIENTE SELECT * FROM CLIENTE WHERE CIU_CODIGO='C-78';
SELECT * FROM CLIENTE WHERE CIU_CODIGO='C-78';
SELECT * FROM HIS_CLIENTE;
COMMIT;
